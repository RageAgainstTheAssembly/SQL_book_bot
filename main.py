...
def menu_command(update: Update, context: CallbackContext) -> None:
    ...
    keyboard = [
        [InlineKeyboardButton("See all books", callback_data='all')],
    ]
...
def button(update: Update, context: CallbackContext) -> None:
    ...
    if query.data == "all":
        cur.execute('SELECT title, id, price FROM bsp.edition WHERE  ORDER BY meanrating DESC NULLS LAST')
...
def cart_command(update: Update, context: CallbackContext) -> None:
    ...
    #get titles in cart
    cur.execute(f"SELECT title FROM bsp.purchase_bookinstance_includes JOIN bsp.edition ON bookinstance_id=id WHERE purchase_id={int(purchase_id)}")
